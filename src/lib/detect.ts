...
export function getDevice(screen: string, os: string) {
  if (!screen) {
    // Default to desktop if no screen info available
    return 'desktop';
  }
  ...
  // If width is not a valid number, default to desktop
  if (isNaN(numericWidth) || numericWidth <= 0) {
    return 'desktop';
  }

  if (DESKTOP_OS.includes(os)) {
    if (os === 'Chrome OS' || numericWidth < DESKTOP_SCREEN_WIDTH) {
      return 'laptop';
    }
    return 'desktop';
  } else if (MOBILE_OS.includes(os)) {
    if (os === 'Amazon OS' || numericWidth > MOBILE_SCREEN_WIDTH) {
      return 'tablet';
    }
    return 'mobile';
  }

  // Fallback logic based on screen width
  if (numericWidth >= DESKTOP_SCREEN_WIDTH) {
    return 'desktop';
  } else if (numericWidth >= LAPTOP_SCREEN_WIDTH) {
    return 'laptop';
  } else if (numericWidth >= MOBILE_SCREEN_WIDTH) {
    return 'tablet';
  } else {
    return 'mobile';
  }
