...
async function handleResponse(res: FetchResponse): Promise<any> {
  if (!res.ok) {
    ...
    if (typeof res.error === 'string' && res.error.includes('<!DOCTYPE')) {
      return Promise.reject(new Error('Server error: Please check your connection and try again'));
    }
    return Promise.reject(
      new Error(res.error?.error || res.error?.message || res.error || 'Unexpected error.'),
    );
  }
